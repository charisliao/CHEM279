...
// ==================== FIND BELOW FOR OPTIMIZED VERSIONS OF THE FUNCTIONS ABOVE ================================
 ...
 * 
 * Key Modifications: 
 * 1. Preallocated memory: The `arma::mat H` is preallocated with final size before computation reduces the need for dynamic resizing during loop. 
 * 2. Off-diagonal elements: since the matrix is symmetric, off-diagonal elems are only computed for j < i.
 * 3. Reuse of Memory: The function now reuses memory for variables like `atom_indexA`, `gammaAA`, and `ZB_gamma_AB`. 
 * 
 */
arma::mat CNDO::computeCoreHamiltonianMatrixOptimized(arma::mat& overlap_mat, std::map<int, BasisFunction> basis_map, vector<string> atom_types, vector<BasisFunction>& basis_set) {
    arma::mat H = arma::zeros(basis_set.size(), basis_set.size());   
    ...
    for (int i = 0; i < basis_set.size(); i++) {
        atom_indexA = basis_set[i].atom_index;  // Reuse memory
        gammaAA = 0.0;
        BasisFunction basisFunctionA = basis_map[atom_indexA];
        gammaAA = gamma(basisFunctionA, basisFunctionA);
        ...
        H(i, i) = diagCoreH(gammaAA, basis_set[i], ZB_gamma_AB);

        for (int j = 0; j < i; j++) {
            // Compute off-diagonal elements (only for j < i to avoid redundant calculations)
            double Beta_A = getBeta(extractElement(basis_set[i].AO_type));
            double Beta_B = getBeta(extractElement(basis_set[j].AO_type));
            double h = computeOffDiagMat(i, j, Beta_A, Beta_B, overlap_mat);
 ...
 * 
 * Key Modifications:
 * 1. Remove creation of basis_map in the function. Instead, pass it in as an argument.
 * 2. Reuse of memory: Reuse memory for variables like `atom_indexA`, `gammaAA`, and `ZA`etc.
 * 
 */
arma::mat CNDO::computeFockMatrixOptimized(std::map<int, BasisFunction> basis_map, vector<string> atom_types, vector<BasisFunction>& basis_set, arma::mat& overlap_mat, arma::mat& Hcore_mat, arma::mat& p, arma::vec& Ptotal) {
    ...
    double Beta_A;
    double Beta_B;
    BasisFunction basisFunctionA;
    BasisFunction basisFunctionB;
    double ZB; 
    double gammaAB;
    double pTotalB;
    double overlap;
    ...
    for (int i = 0; i < basis_set.size(); i++) {
        for (int j = 0; j < basis_set.size(); j++) {
            if (i == j) {
                IA = getIA(extractElement(basis_set[i].AO_type), extractOrbital(basis_set[i].AO_type));
                atom_indexA = basis_set[i].atom_index;
                gammaAA = 0.0;
                basisFunctionA = basis_map[atom_indexA];
                gammaAA = gamma(basisFunctionA, basisFunctionA);
                ZA = basisFunctionA.valence_e;
                pTotalA = Ptotal(atom_indexA);
                ...
                for (int index = 0; index < atom_types.size(); index++) {
                    if (atom_indexA == index) {
                        ...
                        BasisFunction basisFunctionB = basis_map[index];
                        ZB = basisFunctionB.valence_e;
                        gammaAB = gamma(basisFunctionA, basisFunctionB);
                        pTotalB = Ptotal(index);
                        pZB_gamma_AB += (pTotalB - ZB) * gammaAB;
                    }
                }
                ...
                Beta_A = getBeta(extractElement(basis_set[i].AO_type));
                Beta_B = getBeta(extractElement(basis_set[j].AO_type));
                overlap = overlap_mat(i, j);
                density = p(i, j);
                basisFunctionA = basis_map[basis_set[i].atom_index];
                basisFunctionB = basis_map[basis_set[j].atom_index];
                gammaAB = gamma(basisFunctionA, basisFunctionB);
                F(i, j) = offDiagonalFockElement(Beta_A, Beta_B, overlap, density, gammaAB);
            }
        }
